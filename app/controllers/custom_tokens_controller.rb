class CustomTokensController < Doorkeeper::ApplicationMetalController
  def create
    ...
    if response.status == :ok
      # Account the resource_owner_id from token to identify the account
      account = Account.find(response.token.resource_owner_id) rescue nil

      unless account.nil?
        ### If you want to render account with template
        ### create an ActionController to render out the account
        # ac = ActionController::Base.new()
        # account_json = ac.render_to_string( template: 'api/accounts/me', locals: { account: account})
        # body[:account] = Oj.load(account_json)

        ### Or if you want to just append account using 'as_json'
